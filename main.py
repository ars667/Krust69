...
def line2(x1, y1, x2, y2):
    c.create_line(x1, y1, x2, y2, fill="#11f")


#def text1(x2, x1, y2, y1, i, j):
    #c.create_text(
        #(x2 + x1) / 2, (y2 + y1) / 2, anchor=W, font="Arial",
        #text=mas[i][j])
    ...
    for i in range(N):
        for j in range(i, N):
            if mas[i][j] != 10:
                ...
                y2 = sin2 * R + 300
                x2 = cos2 * R + 400
                window.after(0, line1(x1, y1, x2, y2))
                window.update()
...
class Graph:
    ...
    def add_edge(self, u, v, w, angle, R):
        ...
        y2 = sin2 * R + 300
        x2 = cos2 * R + 400
        window.after(100, line2(x1, y1, x2, y2))
